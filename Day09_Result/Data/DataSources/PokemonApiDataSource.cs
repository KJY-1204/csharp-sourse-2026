...
public class PokemonApiDataSource : IPokemonApiDataSource
{
    ...
    public async Task<Result<PokemonDto>> GetPokemonAsync(string pokemonName)
    {
        ...
        {
            return Result<PokemonDto>.Failure("포켓몬 이름이 입력 되지 않음");
        }
        ...
        try
        {
            ...
            {
                return Result<PokemonDto>.Failure($"포켓몬 '{pokemonName}'을 찾을 수 없습니다.");
            }
            ...
            if (dto is null)
            {
                return Result<PokemonDto>.Failure("응답 데이터 역질렬화 불가");
            }
        ...
        catch (HttpRequestException ex)
        {
            ...
            return Result<PokemonDto>.Failure($"네트워크 연결 오류 : {ex.Message}");
        }
        catch (Exception ex)
        {
            // 기타 JSON 파싱 실패 등의 오류 방어
            return Result<PokemonDto>.Failure($"알 수 없는 오류 발생 : {ex.Message}");
        }
    }
}